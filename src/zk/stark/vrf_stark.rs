...
/// VRF STARK prover for creating proofs of correct VRF computation
...
impl VrfStarkProver {
    ...
    pub fn prove(&self, input_data: &[u8]) -> Result<VrfStarkProof, Box<dyn Error>> {
        ...
        let stark_prover = VrfStarkProver::new(trace, constraints)?;
...
/// VRF STARK verifier for checking proofs
...
impl VrfStarkVerifier {
    ...
    pub fn verify(&self, proof: &VrfStarkProof) -> Result<bool, Box<dyn Error>> {
        ...
        let stark_verifier = StarkVerifier::new(constraints, self.security_parameter)?;
...
use crate::zk::stark::stark::{VrfStarkProof, VrfStarkProver, VrfStarkVerifier};
