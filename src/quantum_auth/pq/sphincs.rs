...
impl SphincsAuth {
    ...
    pub fn with_variant(variant: SphincsVariant) -> Result<Self, Box<dyn Error>> {
        ...
        if variant != SphincsVariant::Sha2128f {
            debug!("Requested variant {} not available, using {} instead", 
                   variant.name(), SphincsVariant::Sha2128f.name());
        }
    ...
    pub fn sign(&self, message: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
        ...
        debug!("Generated SPHINCS+ signature: {}B in {:?}", serialized.len(), sign_time);
        Ok(serialized)
    }
    ...
    pub fn verify(&self, message: &[u8], signature_bytes: &[u8]) -> Result<bool, Box<dyn Error>> {
        ...
        let detached_signature = match sphincssha2128fsimple::DetachedSignature::from_bytes(&signature.sig_bytes) {
            Ok(sig) => sig,
            Err(e) => {
        ...
        let start = std::time::Instant::now();
        let result = match sphincssha2128fsimple::verify_detached_signature(&detached_signature, message, &self.public_key) {
            Ok(_) => {
                info!("✅ SPHINCS+ signature verified successfully in {:?}", start.elapsed());
                true
            },
            Err(e) => {
