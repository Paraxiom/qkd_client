...
impl QKDVerifiableRandomFunction {
    ...
    pub async fn generate_with_qkd(&self, input: &[u8], dest_id: &str) -> Result<(Vec<u8>, Vec<u8>, String), Box<dyn Error>> {
    ...
    pub async fn verify_with_qkd(&self, input: &[u8], output: &[u8], proof: &[u8], key_id: &str) -> Result<bool, Box<dyn Error>> {
